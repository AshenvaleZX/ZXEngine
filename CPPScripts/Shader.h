#pragma once
// 自定义Shader构造器，可以用写在硬盘上的Shader代码来建立Shader，简化Shader创建流程，方便切换不同Shader
#ifndef SHADER_H
#define SHADER_H
...
public:
    ...
    // 下面是几个设置矩阵的函数，其中第三个参数询问我们我们是否希望对我们的矩阵进行置换(Transpose)，也就是说交换我们矩阵的行和列。
    // OpenGL开发者通常使用一种内部矩阵布局，叫做列主序(Column-major Ordering)布局。GLM的默认布局就
    // 是列主序，所以并不需要置换矩阵，填GL_FALSE
    // ------------------------------------------------------------------------
