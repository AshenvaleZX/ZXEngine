...
namespace ZXEngine
{
    ...
    {
        zOutError << pCallbackData->pMessage << std::endl;
    ...
    void RenderAPIVulkan::Dispatch(uint32_t commandID, uint32_t shaderID, uint32_t groupX, uint32_t groupY, uint32_t groupZ, uint32_t xSizeInGroup, uint32_t ySizeInGroup, uint32_t zSizeInGroup)
    {
    ...
    // Integer
    void RenderAPIVulkan::SetShaderScalar(Material* material, const string& name, int32_t value, bool allBuffer)
    {
        if (allBuffer)
        {
            vector<void*> valueAddresses;
    ...
    void RenderAPIVulkan::SetShaderMatrix(Material* material, const string& name, const Matrix4* value, uint32_t count, bool allBuffer)
    {
        if (allBuffer)
        {
            vector<void*> valueAddresses;
    ...
    uint32_t RenderAPIVulkan::CreateImage(uint32_t width, uint32_t height, uint32_t mipLevels, uint32_t layers, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VmaMemoryUsage memoryUsage)
    {
        ...
        // 目前所有通过这个CreateImage接口创建的image都是在CreateVulkanTexture里使用，然后在DestroyTextureByIndex里销毁的
        // 如果以后有出现通过CreateImage创建的image通过别的形式使用，引用计数方式可能需要重新设计
    ...
    VkImageView RenderAPIVulkan::CreateCubeFaceImageView(VkImage image, uint32_t face, VkFormat format, VkImageAspectFlags aspectFlags, VkImageViewType viewType)
    {
        VkImageViewCreateInfo createInfo = {};
        createInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
        createInfo.image = image;
